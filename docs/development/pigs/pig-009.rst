...
`MapDataset` object can store information for more than one source.
Then, a `.sample` method will draw the sampled events and will provide an output 
`~astropy.table.Table` object. The latter will containt the reconstructed sky positions, 
energies, times, and an `EVENT_ID` and `MC_ID`. `EVENT_ID` is a unique number or a string 
to identify the sampled event, while `MC_ID` is a unique ID (number or string) to identify 
the model component the event was sampled from.
...
We note that the `MapDatasetSampler` will be designed to work alternatively for input flux or 
counts maps.
...
.. code::
	...
	events = Table('RA', 'DEC', 'ENERGY', 'TIME')
    return EventList
...
will be an `~astropy.table.Table` with columns: `TRUE_RA`, `TRUE_DEC` and `TRUE_ENERGIES`.

Then, the time will be sampled independently. This will be done through a `sample` 
method that we propose to add into `~gammapy.time.models.LightCurveTableModel`
or `~gammapy.time.models.PhaseCurveTableModel`. This method will take as input a 
lightcurve (or phasecurve) model and the GTIs (i.e. one Tstart and Tstop) in the `MapDataset`
object. Also in this case the `InverseCDFSamler` class is the machine used to sample the 
...
is not addressed in this PIG. For the scope of this PIG, the fundamental
class `MapDatasetEventSampler` to simulate events corresponding to a given observation 
