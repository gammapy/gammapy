...
The `InverseCDFSampler` could additionally implement a few helper functions to plot
...
It is important to note that the sampling with binned data put the events at the 
center of the bin, implying that they do not have a smooth distribution. However,
this issue is overcame once the point spread function (PSF) is applied to the data.
...
where `pointing` is an `~astropy.SkyCoord` object, necessary in the case the user
wants to simulate a source in an off-axis position.
...
number or a string to identify the sampled event, while `MC_ID` is a unique ID 
(number or string) to identify the model component the event was sampled from.
