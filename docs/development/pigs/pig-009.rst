...
::
    ...
    obs = Observation(pointing, gti, aeff, psf, edisp)
...
model. Then, for each source, the ``src.evaluate`` method will calculate the predicted 
...
We propose to add a ``MapDataset.sample(n_events=, random_state=)`` method in ``~gamampy.cube.MapDataset`` 
that will be the core of the sampling process. The ``sample`` is based on the 
``~gammapy.utils.random.InverseCDFSampler`` class described in ``GH 2229``_ . The output
...
Tstart and Tstop) in the ``MapDataset`` object. Also in this case the ``InverseCDFSampler`` 
...
The define a light-curve per model component the current ``SkyModel`` class will be
extended by a ``SkyModel(..., temporal_model=)``.

The IRF correction can now be applied to sampled events. We propose to add a 
``EdispMap.sample(events=)`` method in both ``~gammapy.cube.PSFMap`` and ``~gammapy.cube.EdispMap``.
...
assuming a constant event rate. Finally, the IRF corrections are not applied to sampled
events.
