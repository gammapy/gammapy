...
below: `Masks`_). The second place where regions appear is to select objects of
interest (events, observations, sources) for a given region (see section below:
`Tables`_). Finally, there is the use case of rotated regions for background
estimation in  1D spectral analysis (see section below: `Rotated regions`_).
...
scripts to do it using ``astropy-healpix`` and Gammapy.
...
`gammapy/background/reflected.py`_. This is one of the most complex pieces of
...
exclusion mask and WCS in ``reflected.py``. For convenience, we could generate a
dummy WCS as a local TAN projection to the pointing position, if the user
doesn't need of have an exclusion mask. This is a common case e.g. for isolated
point sources like AGN, where the rotated regions method is most commonly used.
...
- Add ``PixRegion.rotate(center, angle)`` in ``astropy-healpix``. See `Rotated regions`_ above.
- Release ``astropy-healpix`` v0.4, then bump Gammapy dependency to that version.
- Improve rotated region code and tests and docs in Gammapy.
  This could mean a rebase and continuation of `GH 2089`_, or closing that and re-starting from master,
  copying over the useful parts or cherry-picking useful commits from that branch.
- Add ``gammapy/utils/regions.py`` with two helper functions. See `Region arguments`_ above.
- Use the helper functions throughout Gammapy. Should be 5-10 cases, could be several PRs. See `Region arguments`_ above.
- Replace sky circle and box select with generic region select (see `Sky and pixel regions`_, resolves `GH 1172`_)
...
- As mentioned in `Sky and pixel regions`_, there are a few options to define the semantics for
contains and plot.
...
- As mentioned in `Sky and pixel regions`_, those methods aren't really needed, they are just a
...
.. _gammapy/background/reflected.py: https://github.com/gammapy/gammapy/blob/master/gammapy/background/reflected.py
