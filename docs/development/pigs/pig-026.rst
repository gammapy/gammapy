...
1. ``Prior`` base class and subclasses
2. ``.prior`` attribute in the ``Parameter`` and ``Model`` classes
3.  ``PriorFitStatistic`` class 
...
--------------
The goal is to fit energy-dependent systematics in the FoVBackground with nuisance parameters. The correlation between the energy bins and the magnitude of the expected systematic uncertainty has to be accurately estimated and serves as an input to the covariance matrix. Combining the ``PieceWiseNormSpectralModel`` with a prior set and the ``PowerLawNormSpectralModel`` the FoVBackgrounds ``norm`` and ``tilt`` and in addition, the systematics can be fitted. 
The first step consists of defining the energy bins in which the systematics is fitted and setting up the ``PieceWiseNormSpectralModel``:


.. code:: python

  energy = dataset.geoms['geom'].axes[0].center
  sys_model = PiecewiseNormSpectralModel(
      energy= energy,
      norms=[0]*len(energy),
  )

For the combination of the ``sys_model`` and the classical used ``PowerLawNormSpectralModel``, a ``NormCompoundSpectralModel`` is used and set as the ``spectral_model`` of the ``PriorFoVBackgroundModel``:


.. code:: python
  
  bkg_spectral = NormCompoundSpectralModel(PowerLawNormSpectralModel(),
                                      sys_model,
                                      operator=operator.add)
  bkg = PriorFoVBackgroundModel(spectral_model = bkg_spectral,
                          dataset_name = dataset.name)

Now the prior is defined and set. Since there is a correlation of the systematics between the energy bins, the ``CovarianceGaussianPrior`` is used with a Gaussian covariance matrix.
...
.. code:: python
  ...
  for i in range(l):
      if i > 3:  # no correlation between the PowerLawNormSpectralModel parameters
          cov[i,:] = norm.pdf(range(l) , loc = i , scale = 1 ) /0.4 * 0.1 
  
  prior = CovarianceGaussianPrior(cov = cov)
  bkg.prior = prior

A preliminary version was set up like this and tests on simulated datasets were successful.

Case 2: Support unfolding methods for spectral flux points `#4122`_
--------------

The proposed prior class allows the probability to unfold spectral flux points with Tikonov regularisation. The Tikonov matrix can be defined and set as the covariance matrix in the  ``CovarianceGaussianPrior``. The weight of the ``PriorFitStatistic`` can be interpreted as the regularisation strength tau.
