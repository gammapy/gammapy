...
* Which "convenience functions" similar to the ``MapDataset``:
  ...
  * slice_by_energy: Needs slight modifications. We could slice the mask and group the events according to the slices. Or build a new mask based on the slices. Maybe useful for the computation of flux points.
...
* Returns differential model flux at event's position, the total model flux inside the mask. Returns [``numpy.array``, float] where the array shape corresponds to (N_event,) and N_event is the number of events contributing to that model.
