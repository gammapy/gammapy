...
1. Design and develop a High-level interface API
---------------------------------------------

We will develop a High-level interface Python API which uses a param-value configuration file 
...
- Within an IPython session or notebook using a manager object to perform specific tasks in an analysis process
...
data products and logging. It is flexible enough to allow the user to work with the API at any stage of the
analysis process, and not only from the very start to the very end.  

**Use cases**

The use cases covered are in the scope of a single analysis and model, not parameterised variations, within a single region (e.g. 10 deg region with 5 sources).

The main use cases for analysis forseen:
...
To generate the config file, we could add a ``gammapy analysis config`` which dumps the config file with all lines commented out, and the user can then fill in the numbers they care about. As an alternative users could copy & paste from config file example in the docs.
...
- The Tools classes
Middle management agents (e.g. MapMaker, ReflectedBgEstimator, ...)

- Control dispatching among manager and tools
...
TODO /CLARIFY
        ...
        Design how these interact, not clear who drives what. Very important: what's the responsibility of `Analysis` vs `Observations` vs `Datasets` vs `Fit`. How do we compose lower-level Tools into Analysis chains? Who creates the Tool objects when? Analysis needs to know how to turn config into Python objects. E.g. for geom, but also for exclusion_mask: exclusion.fits, make a Map object. 
...
- Serialisation will be a mix of YAML and FITS files-
...
We could add a ``gammapy analysis data_reduction`` or ``gammapy analysis optimise`` which does the slow parts before going to ipython or jupyter, using all the information from the config file.

**Commnad Line Tools**

- ``gammapy analysis config``
- ``gammapy analysis data_reduction``
- ``gammapy analysis optimise``
...
2. Explore the use of papermill to run workflows driven by parametrized notebooks
------------------------------------------------------------------------------
