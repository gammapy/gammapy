...
YAML files placed in the ``download/tutorials`` folder of the [Gammapy Web-page Github
repository](https://github.com/gammapy/gammapy-webpage). The same happens for conda 
working environments of stable releases declared in files placed in the ``download/install`` 
folder of that repository. The datasets are not versioned and are similarly declared in 
