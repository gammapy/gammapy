...
second ``gammapy-extra`` repository. The motivation was to keep the main
...
developers and as one place where datasets can be put, but it will not be mentioned to users.
...
other datasets not used in the tutorials are shipped to users.  Possible solutions to
this and related problems have been discussed serval times, see e.g. `GH 1237`_, `GH 1369`_, 
...
We count on developer documentation and code review to guarantee empty-output 
notebooks stored in ``gammapy/notebooks``, though we can also explore 
`nbstripout`_ for a potential implementation of an automated mechamism to 
remove outputs from notebooks in the Github repository.

When preparing for the Sphinx documentation building one can tweak some parameters 
in the ``setup.cfg`` file to execute notebooks during the process and get the static 
notebooks with output cells rendered in the the documentation. On the contrary, 
links to Binder notebooks and download of *.ipynb files will point to empty-output 
notebooks.
...
command will download all notebooks and the input datasets related. Not output datasets
from the notebooks will be downloaded. All files will be copied into a 
``$CWD/gammapy-tutorials`` folder, the datasets placed in a ``datasets`` subfolder and 
the notebooks into a ``v.x.x`` subfolder accounting for the version downloaded. The URLs 
of the input files used by the notebooks should be noted in the ``notebooks.yaml`` file.
The management of updating the ``gammapy-tutorials`` folder after a local update of ``gammapy`` 
is left up to the user. We could also implement a function with some check and version
printout that we put in the first cell at the top of each notebook.

One issue with ``gammapy download`` is that one cannot go back and fix or change something
