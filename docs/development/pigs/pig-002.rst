...
data reduction for IACT from DL3 inputs (event lists and IRFs) to DL4 (3.5?) data (spectra, maps and cubes)
...
currently in gammapy to produce maps and cubes (mostly developpedby @adonath and @ljouvin).Image  processing can be performed
...
allows to get rid of the cross dependencies of the image and cube package we have now. 
 ...
 The existing code will remain in gammapy for the moment, with possibly some bugs fixed. The new code
is largely independent so that the new developement should bot break user scripts.


Decision
========

This PIG has been approved and a first implementation been performed in PR #1314.
