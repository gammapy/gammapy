...
across multiple IACT observations, joint-likelihood fitting of IACT and Fermi-LAT
...
For modeling of flux points we propose to introduce a `FluxPointsDataset`:
...
The `FluxPointsDataset` should define at least the following methods:
...
be supported by a specific option `FluxPointsDataset(likelihood="template")` or similar.
...
9. Add a `name` attribute to datasets and a `Datasets.__getitem__` method (**v0.12**).
