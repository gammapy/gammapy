...
All dataset objects should implment a `__str__` method, so that the following
works:
...
13. Implement `MapDataset.fake()`, `SpectrumDataset.fake()` and `SoectrumDatasetOnOff.fake()` methods (**v0.14**).
...
Decision
========
Because of the urgent need of the dataset abstraction for API unification
and joint-likelihood analyses, the proposal in this PIG has been implement
without official sign off by the Gammapy coordination committee, but after
approval by the lead developers.
