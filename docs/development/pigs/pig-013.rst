...
Dependency       Gammapy 0.12     Gammapy 0.14
===============  ================  ================
ipython=7.5      7.3   (Feb 2019)  7.6   (Jun 2019)
...
add it to the recommended conda environment. We propose to continue prototyping MCMC s
...
With ``iminuit``, 1.3.7 is the first version that ships binary wheels.

TODO: for ``reproject`` it's not clear if we actually use or need it. Probably
``map.reproject`` exactly duplicates the functionality of ``map.interp``, except
that ``reproject`` additionally offers the possibility to reproject using a
flux-preserving algorithm. We should check exactly what we have and need and
then decide whether to keep or drop ``reproject`` as a dependency.
...
TODO: now I'm having second thoughts. Maybe we should go the conda metapackages
route and for pip either ``extras_require`` or a metapackage?

More examples;
- Metapackages for pip and conda: https://pysal.org/install
- Relevant link: https://docs.conda.io/projects/conda-build/en/latest/resources/commands/conda-metapackage.html
- Looks like Sunpy uses ``extras_require``, see https://docs.sunpy.org/en/stable/guide/installation/advanced.html#sunpy-s-requirements
- Looks like Glue uses ``extras_require`` for pip (see http://docs.glueviz.org/en/stable/installation/pip.html),
  but then for conda has a metapackage ``glueviz`` https://github.com/conda-forge/glueviz-feedstock/blob/master/recipe/meta.yaml
  which depends on ``glue-core``.
...
Mention HomeBrew, Docker, Linux distros

Outlook
=======

Possible somewhat related future discussion and work, not in scope for this PIG:

- Adopt new packages and technologies for new features or improved performance,
  e.g. ``numba``, ``dask``, ``jax``, ``ray``, ``tensorflow-probability``, ...
- Simplify Gammapy packaging code, possibly get rid of ``astropy-helpers`` completely.
  See  https://github.com/astropy/astropy-APEs/pull/52
- Change git and release workflow (stable branches, bugfix backports) after v1.0
  Better continuous integration, possibly set up continuous delivery.

Alternatives
============

- We could just keep doing what we are now: support more older versions, use healpy,
  keep support for Macports
- We don't have to add more support for pip and produce binary wheels,
  conda could remain our one recommended and fully supported distribution channel
- We could use a conda metapackage (and possibly pip metapackage) instead of
  environment specification files via gammapy.org
- We could package for HomeBrew which now works on Linux and Mac
- We could produce Docker images
