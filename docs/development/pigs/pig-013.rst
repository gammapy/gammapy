...
via setuptools, so that ``pip install gammapy[all]`` installs the optional
Gammapy dependencies. We suggest to remove the extensive installation
instruction page for Macports from the Gammapy docs, and to not include
Macports, HomeBrew or Linux distributions in the distribution channels that are
maintained by the Gammapy team.
