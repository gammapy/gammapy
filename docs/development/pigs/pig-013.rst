...
requirements`_ file from now on, i.e. in addition to
...
widely used by Python projects. For Gammapy it would mean we'd have to change to
a different package name, e.g. ``gammapy-core``, to make room for ``gammapy`` to
be the metapackage, or to add a metapackage with a name like e.g.
``gammapy-recommended`` or ``gammapy-env`` or something like that. While this is
a nice solution, we  suggest to stick to ``gammapy`` as a package with a very
small set of dependencies, and then use the environment specifications to create
the stack and execution environment that most users want, not metapackages for
now.
