...
We note that we expect the Gammapy dependencies to change from time to time.
Especially within the next year or two, eventually Gammapy will mature and
settle down. E.g. if Astropy changes to use ``ruamel_yaml`` instead of
``pyyaml``, we would likely follow. Another change we will likely make is to use
``astropy-healpix`` instead of ``healpy`` once ``astropy-healpix`` has matured.
We also plan to work on performance in Gammapy next year, and will consider
using ``numba``. The dependencies listed in this PIG are the current status,
each change will be considered and discussed via a pull request or future PIG.
