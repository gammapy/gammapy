...
Linux, macOS and Windows, so that users don't need a compiler and installation
is fast. We suggest to define a list of dependencies "all" in ``extras_require``
via setuptools, so that ``pip install gammapy[all]`` is mostly equivalent to
installing the recommended conda environment, whereas ``pip install gammapy``
only installs the few required dependencies, as it already did so far,
equivalent to ``conda install gammapy``. We suggest to remove the extensive
installation instruction page for Macports from the Gammapy docs, and to not
include Macports, HomeBrew or Linux distributions in the distribution channels
that are maintained by the Gammapy team.
...
Currently we have both ``pip install gammapy`` and ``conda install gammapy``
only install the required dependencies. We suggest to keep it like that.
...
It is not described here, but very likely after implementing this PIG we will
follow up by improving Gammapy testing (`GH 2270`_) and modernise the Gammapy
packaging following the recommendations in `APE 17`_, and we might also change
the Gammapy git and release workflow to have stable branches with backports
after v1.0.
...
Maybe move completely from travis-ci to Azure?
...
- Maybe: rewrite spectral model error evaluation code to not use the
``uncertainties`` package (`PIG 14`_)
...
extensive testing before announcing to users. Maybe define some Dockerfiles that
install from scratch from the user's perspective, to be used for post-release
checks?
