...
So far, the only API in gammapy handling these differents flux types is the ``FluxPoints``
...
associated quantities and other informations. It could be useful to export the
...
So we propose to introduce the following simplifed estiomator API:

- `FluxPointsEstimator`, which handles multiple sources, time intervals and energy bins, using "foward folding"
- `FluxMapEstimator`, which handles multiple energy bins, using "foward folding"
