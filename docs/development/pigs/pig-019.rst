...
Minor changes
-------------
In addition propose to move the ``PSFKernel`` and ``EDispKernel``,
``EDispMap`` and ``PSFMap`` to either ``gammapy.maps`` or ``gammapy.irf``.
...
and registries. Introduce a ``subpackage/plotting.py`` file,
where plotting related functionality lives.
