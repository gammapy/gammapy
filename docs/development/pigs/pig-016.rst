...
- gammapy.irf and gammapy.spectrum and most containers in Gammapy might be changed to maps.
- gammapy.spectrum will likely become higher-level than gammapy.cube,
  i.e. 1D spectral analysis will be based on 3D maps analysis more.
...
- we might rename gammapy.cube to gammapy.map_analysis or gammapy.scripts
  to gammapy.analysis or similar changes depending on how their purpose and
  scope changes in the future
- datasets could be grouped in gammapy.datasets, if they are simple container objects
  (like models) that aren't tightly coupled with analysis code in
