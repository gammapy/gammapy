...
a policy about authorship is necessary. This PIG adresses this issue by setting an authorship policy for the
Gammapy project for each type of products.

1. Introduction
===============

Gammapy has started in 2013 and is now widely used in scientific publications. A proper citation scheme with a correct
authorship allows:

- a proper referencement of the used Gammapy release,
- a proper recognition of the achieved work of any contributor,
- a  compliance with the FAIR principles for Reseach Software (`FAIR4RS <https://www.rd-alliance.org/group/fair-research-software-fair4rs-wg/outcomes/fair-principles-research-software-fair4rs>`_).
...
our web pages; then, the authorship policy is given for each of products associated to Gammapy, namely the intermediate
releases, the Long Term Support (LTS) releases, the general Gammapy papers, the contributions in conference.

2. Reminder of the citation scheme
==================================

When Gammapy is used for any publication, contribution in conferences or software, authors should cite properly Gammapy.
It is asked to cite the DOI of the latest Gammapy paper.

.. note::
    As the Gammapy community fully supports the Open Science, we strongly encourage authors using Gammapy to follow
    the FAIR4RS principles and to allow the reproducibility of their results. As consequence, we suggest to always mention
...
This scheme and these two references are precised in our web pages.

3. Authorship policy
====================

The Gammapy references contain a list of authors, or `contributors`, that requires to be updated with time according to a
general policy. This section defines what is a contributor to Gammapy and the policy for each type of products.
...
- adding or updgrading lines of our Python code,
- adding or improving the documentation (rst pages, docstring, gammapy-webpage),
- updating or improving our DevOps environment (CI, GitHub configurations, gammapy-docs, release processes, etc),
- adding or upgrading our associated projects (gammapy-benchmarks, gammapy-data, gammapy-extra, gammapy-cat, gammapy-recipes),
...
- organising and leading hand-on sessions or schools,
...
.. important::
    We strongly recommend that the contributors use always the same GitHub account, to have an unique
    `ORCID <https://orcid.org/>`_ identifier and store it into your GitHub user profile, and to inform the developers
    of any change of affiliation.

As Gammapy is an open project, we encourage users to provide feedbacks on the usage of Gammapy, on features, on issues
via our communication channels.

Definition of the `Maintainer`
------------------------------

In the Open Source community, a maintener is generally mentioned in metadata associated to a release and has to given.
...
people that made a contribution as listed in the previous sub-section to the current, and to the previous release in
order to account for long contributions (e.g. management of depreciated features, improvement/debugging of features,
etc). Any additional author can be inserted in the list for its special contribution when requested by the Gammapy
Coordination Committee.

The order of the authors is first 'The Gammapy Collaboration', the Lead Developpers, a past Lead Developer in case of
change between two releases, followed by the list of contributors in alphabetical order.

This policy is applied to any realease after the LTS V1.0. For the releases made before, an hand-made list is used.
...
The list of authors is composed by the union of all the contributors of the releases realised up to the last LST
release. Any additional author can be inserted in the list for its special contribution when requested by the Gammapy
Coordination Committee.
...
A.1/ By default, 'The Gammapy Collaboration' is mentionned in first, then the Lead Developpers, then the past Lead Developers since the
...
- 'The Gammapy Collaboration', and the list primary paper contributors in order of contribution agreed per consensus with the developement team and the Coordination Committee, as '(Primary Paper Contributors)'
- the members of the Coordination Committee, as '(Gammapy Coordination Committee)'
- the list of contributors of each release since the last Gammapy publication, ordered by alpabetical order, as '(Gammapy Contributors)'
...
One has the freedom to precise dedicated acknowledgements
associated the publication. In partical, it is recommanded to precise the grants or fellowships given to some authors.
One should in any case always acknowledge the Astropy project, to which we are affiliated.

Contributions in conferences
-----------------------------

The section concerns any contribution in conferences, ie a talk, a poster and their associated proceedings, related to
the Gammapy project as it-self. It does not concern a technical or scientific work that uses Gammapy as open library.
In this case, the standard citation scheme should be used.

As the lenght of the author list is generally a constrain, the author list can be reduced to: the short list of authors,
...
Depending of the software repository, different metada files are used in the eco-system of Open Source research
software.

The file ``codemeta.json`` is used by HAl and Software Heritage, and recommanded by
...
In order to respect these rules, some automation is required in order to create the list of contributors. One could use
the Python library `Tributors <https://con.github.io/tributors/>`_. This library uses the GitHub API to determine the list of contributors, that is written in the
format associated to codemeta.json, citation.cff, zenodo.json. This kind of library allows then two requirements:
retrieve automatically the list of contributors from GitHub and write the authors list in all the needed format.

One should note that the user profile of the GitHub platform does not contain a field associated to the ORCID identifier.
...
Even if this librairy is chosen, a specific Python script had to be written such that the authors order follows these
rules. In this case, an integration within a GitHub action could be possible.

6. Suggestions
==============

One could ask to CTAO software coordinators (e.g. DPPS, ACADA) if such rules can be used by them also, even if the
Gammapy project is independant. In the same spirit, advices could be asked to people of the ESCAPE project via our
corresponding person. And finally, one could mention the existance of theses rules and ask for advice to the Astropy
project.
