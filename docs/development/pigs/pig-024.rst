...
- contributing to our associated projects (gammapy-benchmarks, gammapy-data, gammapy-extra, etc),
- contributing to the maintenance of Gammapy products on open-source repositories (conda-forge, PyPI, Zenodo, HAL, docker-hub, etc) or open hub (openhub, etc),
- leading the precise tasks identified by the Lead Developers (e.g module maintainer),
- organizing and leading hands-on sessions or schools,
- managing our social media accounts,
- and coordinating the project by the Lead Developers.

Any contribution without threshold on these topics renders a user as official Gammapy contributor. Many of these
contributions are tracked using the git history. However, the use of personal data is regulated in Europe and one
should have the authorisation of users to be cited with their personal information for publications (their full name
are mandatory, and if any their affiliation and their ORCID number). For this reason, we propose to use the
`Developer Certificate of Origin` (DCO, see the file ``DeveloperCertificate.rst``) for each commit. Its acceptation by
contributors will permit to:
...
The file ``README.md`` will contain a `Contributing` section explaining that the project follows this DCO, as well
as a link to this PIG.

.. important::
    For practical reasons, the developers strongly recommend that the contributors use always the same GitHub account
...
The list of authors is composed of people who contributed to the current release (see :ref:`_contributor`) and who have
accepted the DCO. If the DCO is not accepted by a contributor, their contribution might not be usable and thus
this user will not be in the list of authors per default.
...
to add any additional author into the list for their special contribution. The Project Managers and Lead Developers
...
release. As an LTS release will be associated with a paper, the members of the Coordination Committee will appear in this
...
5.2 Collection of the personal information of authors
-----------------------------------------------------

The data collected from the git logs are not sufficient for any publication. Most of the contributors must give their
...
In this purpose, one could use the public user profile of the GitHub platform. They are public data, but one should
aware that they are not complete and/or up-to-date. As mention earlier, it is recommended to fill correctly this GitHub
profile to help the dev team to pre-filled the metadata files.
...
A second script should be settled in order to create the list of contributors in HTML format that should be
...
5.3 Handling of conference material
-----------------------------------
