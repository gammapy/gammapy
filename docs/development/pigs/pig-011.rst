...
the new :class:``LightCurveEstimator`` class, which  should essentially be a
wrapper around the ``gammapy.spectrum.FluxPointEstimator`` class that does the
same thing for spectrum and map datasets.
...
The new :class:``LightCurveEstimator`` class should therefore be able to take as
