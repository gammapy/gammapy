...
the ``gammapy-0.11`` conda environment:

.. code-block:: bash

    curl -O https://gammapy.org/download/install/gammapy-0.11-environment.yml
    conda env create -f gammapy-0.11-environment.yml
...
.. code-block:: bash
    ...
    gammapy download tutorials --release 0.11
    cd gammapy-tutorials
    export GAMMAPY_DATA=$PWD/datasets-0.11
