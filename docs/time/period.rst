...
3. The spectral window function shows two prominent peaks around one and 27 days.
    ...
    Thus, aliases are expected to appear at :math:`f_{{alias}} = f_{{true}} + n f_{{window}}` for integer values of :math:`n`.
    For the peak in the spectral window function at :math:`f_{{window}} = 1 day^{{-1}}`, this corresponds to the third highest peak in the periodogram at :math:`p_{{alias}} = 0.796`.
...
For the bootstrap, the number of resamplings can be raised to obtain a higher resolution in the resampled distribution of periodogram peaks.
