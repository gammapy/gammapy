...
Notable new features include support for CosmiXs dark matter spectral models, for multiple OFF regions in the theta square plots, for simultaneous flux point extraction for a collection of sources. Extracting flux points from datasets generated from different telescope data is now allowed, but should be used with care as responses and the associated systematics might be strongly different.
