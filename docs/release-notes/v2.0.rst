...
with the ``"sensitivity"`` option. The calculation is based on the Asimov dataset following the method described in `~gammapy.estimator.ParameterSensitivityEstimator`.
...
- The restructuring of statistics and `Dataset` has started with the addition of ``FitStatistic`` classes 
  and a ``FIT_STATISTIC_REGISTRY`` to allow configuration of the statistic on a dataset.
...
- Helper method for observation selection at ``Datastore`` level, specially usful for WCD
