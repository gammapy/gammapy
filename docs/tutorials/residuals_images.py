...
def compute_flux_error(gamma_true, gamma_reco, method):
    ...
    flux_true = power_law_eval(energy, diff_flux_ref*np.ones_like(energy), np.array(gamma_true).reshape(energy.shape), energy_ref*np.ones_like(energy))
