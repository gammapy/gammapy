...
#
# - **1D (spectral, region-based)**. In **1D**, all spatial information is collapsed into a single region (typical a circle around the target, with ON/OFF for the background).
...
# ### 1. Signal model
# The first thing that we need for simulation is a signal model. This signal is obtained according to the expected gamma ray flux from the target source. Hence, we need to define the energy range for the simulation and the spatial resolution of our World Coordinate System (WCS) map, along with the spatial and spectral configuration of the source.
...
# Here we show an example of the differences between both approaches, where the Asimov data is smooth while the MC shows fluctuations. Please go to the Data inspection section to find more inspection methods.
...
#
# In this case, like in the simulations section, we will follow a 3D approach, we manage the data using Spatial+Spectral data, rather than 1D, using only spectral. for more details about these approaches check epigraph Dataset above, since the procedure is the same. Hence, the steps to follow are:
#
...
#
#     >**Note**. Since we are working with real data, we don't have to create the counts, but to compute it. to do so, we have to indicate the "counts" clasue to the maker, so they compute when running.
#
...
#
# As seen in previuos sections, the `peek()` method provides a quick overview of the dataset in a single figure. Additionally, here show these different visualization methods:
#
