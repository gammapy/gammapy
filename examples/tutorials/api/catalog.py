...
# -----------------------
# 
# ``~gammapy.catalog`` contains a catalog registry ``CATALOG_REGISTRY``,
...
# (e.g. ``SourceCatalog3FGL``) directly. The two ways are equivalent, the
...
# ``~gammapy.catalog`` classes, such as e.g. selecting sources by sky
...
# directly the sources positions as a ``SkyCoord`` object (we will show an
...
# ``SourceCatalogObject``. In order to select a source object index into
...
# ``.to_models()`` method of the catalog objects. Here for example we
# create a ``Models`` object from the 4FGL catalog subset we previously
# defined:
# 
...
# ---------------------------------
# 
# Using the ``.to_models()`` method for the
...
# accessed via the propoerties ``.table_large_scale_component`` and
...
# ``gammapy.spectrum.FluxPoints`` object.
# 
