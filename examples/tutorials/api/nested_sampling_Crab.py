...
Perform a Bayesian analysis of a 1D spectrum using a nested sampling technique and plotting the posterior distributions.
...
# In gammapy, we used the NS implementation of the UltraNest package, one of the leading package in Astronomy (already used in Cosmology and in X-rays).
...
# The Crab spectra will be fitted here with a simple powerlaw for simplicity.
#
#
#
...
###############################################################################
# <span style="color:red">**!!Priors!!: unlike a traditional fit where defining priors on the parameters is optional, here it is inherent to the Bayesian approach and are therefore mandatory.**</span>
#
# In this case we will set (min,max) prior that will define the boundaries in which the sampling will be performed.
# Note that it is usually recommended to use a `LogUniformPrior` for the parameters that have a large amplitude range like the `amplitude` parameter.
# A `UniformPrior` means that the samples will be drawn with uniform probability between the (min,max) values in the linear or log space (`LogUniformPrior`).
...
# --------------------------------
#
# As for the `Fit` object, the `Sampler` object can receive different backend (although just one is available for now).
# The `Sampler` comes with "reasonable" default parameters but you can change them via the `sampler_opts` dictionnary.
# Here is a short description of the most relevant parameters that you could change :
#
# - `live_points`:
# - `frac_remain`:
# - `log_dir`:
...
###############################################################################
# Next we can run the sampler on a given dataset using `.run()` in the same way that the `Fit` object. No options are accepted in the run method.

result_full = sampler.run(datasets)

###############################################################################
# In the Jupyter notebook, you should be able to see an interactive visualization of how the parameter space shrinks which starts from the (min,max) shrinks down towards the optimal parameters.
#
# The output above is filled with interesting information which are not easy to understand. Here we provide a short description of the most relevant information provided above.
#
# **During the sampling**
# - Z=-68.8(0.53%) | Like=-63.96..-58.75 [-63.9570..-63.9539]*| it/evals=640/1068 eff=73.7327% N=200
#
#
# **Final output**
#
...
###############################################################################
# Within a Bayesian analysis, the concept of best-fit has to be viewed differently from what is done in a Minuit fit.
#
# The output of the Bayesian analysis is the posterior distribution and there is no "best-fit" output.
# One has to define based on the posteriors what we want to consider as "best-fit" and several options are possible:
#
# - the mean of the distribution
# - the median
# - the lowest likelihood value
#
# Be default the `DatasetModels` will be updated with the `mean` of the posterior distributions.

result_full.models

###############################################################################
# The Sampler class returns a very rich dictionnary.
# The most "standard" information about the posterior distributions can be found in  :
#
...
###############################################################################
# While the above plots are interesting, the real strength of the Bayesian analysis is to visualize all parameters correlations which is usually done using "corner plots".
...
#
#
# c.add_chain(create_chain(result_full.samples, "full"))
# c.add_chain(create_chain(result_0.samples, "0", "g"))
# c.add_chain(create_chain(result_1.samples, "1", "b"))
# c.add_chain(create_chain(result_2.samples, "2", "y"))
# fig = c.plotter.plot()
