...
def test_read_events_cta_1dc(data_store_dc1):
    ...
    assert_allclose(info["TSTART"], 664502400.0 * u.s)
    assert_allclose(info["TSTOP"], 664504192.0 * u.s)
    assert_allclose(info["ONTIME"], 1800.0 * u.s)
    assert_allclose(info["LIVETIME"], 1764.0 * u.s)
    assert_allclose(info["DEADC"], 0.98000001907)
    assert info["TELESCOP"] == "CTA"
    assert_allclose(info["RA_PNT"], 267.68121338 * u.deg)
    assert_allclose(info["DEC_PNT"], -29.6075 * u.deg)
