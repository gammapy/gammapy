...
@requires_data()
def test_yaml_io_3d():
#    filename = "$GAMMAPY_DATA/tests/models/examples.yaml"
    ...
    assert pars0["lon_0"].value == -50.
    assert pars0["lon_0"].unit == "deg"
    assert pars0["lon_0"].max == 180.
    assert pars0["lon_0"].min == -180.
    assert pars0["lon_0"].frozen is True

    assert pars0["lat_0"].value == -0.05
    assert pars0["lat_0"].unit == "deg"
    assert pars0["lat_0"].max == 90.
    assert pars0["lat_0"].min == -90.
    ...
    assert model2.spectral_model.values.data.tolist() == [2.52894e-06,1.2486e-06,6.14648e-06]
    ...
    assert model2.spatial_model.parameters["norm"].value == 1.
    ...
    models_dict = models_to_dict(models,do_cut=True)
