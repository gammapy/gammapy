...
__all__ = [
    ...
    'lon_lat_circle_mask',
    'lon_lat_circle_mask_from_table',
    'make_empty_image',
    'make_header',
...
def lon_lat_circle_mask_from_table(lons, lats, table):
    """Produces a boolean mask for as set circles specified in an astropy table.
    ...
    lats : `~numpy.ndarray`
        Array of latitude values.
    table : `astropy.table`
        Table containing 3 columns: center_lon, center_lat, radius

    Returns
    -------
    mask : `~numpy.ndarray`
        Boolean mask array for a set of circular sub-regions
    """
    
    mask = np.zeros(lons.shape, dtype=bool) 
    for row in table:
        center_lon, center_lat, radius = row
        val = lon_lat_circle_mask(lons, lats, center_lon, center_lat, radius)
        mask = np.logical_or(mask, val)
