...
def test_image_lat_profile():
    ...
    # Test output
    lat_profile1 = profile.image_profile('lat', image, lat, lon, binsz,
                                         errors=True)
    ...
    lat_profile2 = profile.image_profile('lat', image, lat, lon, binsz,
                                         counts, errors=True)
    # atol 0.1 is sufficient to check if correct number of pixels are included
    assert_allclose(lat_profile2['BIN_ERR'].data,
                    44.721359549995796 * np.ones(39), rtol=1, atol=0.1)

    lat_profile3 = profile.image_profile('lat', image, lat, lon, binsz, counts,
...
def test_image_lon_profile():
    ...
    mask_array = np.zeros_like(image.data)
    # Test output
    lon_profile1 = profile.image_profile('lon', image, lat, lon, binsz,
                                         errors=True)
    # atol 0.1 is sufficient to check if correct number of pixels are included
    ...
    lon_profile2 = profile.image_profile('lon', image, lat, lon, binsz,
                                         counts, errors=True)
    # atol 0.1 is sufficient to check if correct number of pixels are included
    assert_allclose(lon_profile2['BIN_ERR'].data,
                    31.622776601683793 * np.ones(79), rtol=1, atol=0.1)

    lon_profile3 = profile.image_profile('lon', image, lat, lon, binsz, counts,
                                         mask_array, errors=True)
