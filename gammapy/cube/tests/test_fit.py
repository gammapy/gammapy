...
def test_map_fit(sky_model, geom, geom_etrue):
    ...
    assert_allclose(pars.error(pars[8]), 0.015399, rtol=1e-2)

    # background norm 2
    assert_allclose(pars[11].value, 1, rtol=1e-2)
    assert_allclose(pars.error(pars[11]), 0.02104, rtol=1e-2)
...
def test_stack(geom, geom_etrue):
    ...
    assert_allclose(dataset1.counts.data.sum(), 8000)
    assert_allclose(dataset1.background_model.map.data.sum(), 6000)
