...
def make_separation_map(ref_geom, position):
    ...
    tmp_coords = ref_geom.to_image().get_coord()
    X = tmp_coords[0]
    Y = tmp_coords[1]
    
    ## The first two coordinates are spatial
    skycoords = SkyCoord(X,Y,frame=frame,unit='deg')
...
class MakeMaps(object):
    ...
    def __init__(self, ref_geom, offset_max):
        ...
        self.exposure_map = WcsNDMap(self.ref_geom, data=np.zeros_like(self.count_map.data)* u.m**2*u.s)
        self.background_map =  WcsNDMap(self.ref_geom, data=np.zeros_like(self.count_map.data)* u.Unit(''))
