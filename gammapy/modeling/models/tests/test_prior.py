...
def test_lognormal_prior_evaluate_at_center():
    ...
    prior = LogNormalPrior(log10_obs=19.3, sigma_stat=0.1)
...
def test_lognormal_prior_sigma_in_quadrature():
    ...
    prior = LogNormalPrior(log10_obs=19.3, sigma_stat=0.3, sigma_syst=0.4)
    assert_allclose(prior.sigma_total.value, 0.5, rtol=1e-6)


def test_lognormal_prior_only_syst():
    prior = LogNormalPrior(log10_obs=19.3, sigma_syst=0.2)
...
def test_lognormal_prior_invalid_sigma_stat():
    with pytest.raises((TypeError, ValueError)):
        LogNormalPrior(log10_obs=19.3, sigma_stat=-0.1)

    with pytest.raises((TypeError, ValueError)):
        LogNormalPrior(log10_obs=19.3, sigma_stat="bad")


def test_lognormal_prior_invalid_sigma_syst():
    with pytest.raises((TypeError, ValueError)):
        LogNormalPrior(log10_obs=19.3, sigma_syst="bad")

    with pytest.raises((TypeError, ValueError)):
        LogNormalPrior(log10_obs=19.3, sigma_syst=-0.1)
...
def test_lognormal_prior_sigma_none_converts_to_zero():
    prior = LogNormalPrior(log10_obs=19.3, sigma_stat=None, sigma_syst=None)
