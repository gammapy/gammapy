...
def test_center():
    ...
    kernel = Gaussian2DKernel(stddev=3, x_size=x_size, y_size=y_size).array
...
def test_image():
    ...
    # Create test dataset
    kernel = Gaussian2DKernel(stddev=3, x_size=x_size_kernel, y_size=y_size_kernel).array
    excess = total_excess * Gaussian2DKernel(stddev=3, x_size=x_size_image, y_size=y_size_image).array
    background = total_background * ones / ones.sum()
    counts = excess + background
    #np.random.seed(0)
