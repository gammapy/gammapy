import numpy as np
from sherpa.astro.ui import get_model, get_bkg
#from gammapy.poisson import significance
...
class SpecModel():
    ...
    def get_plot_arrays(self,data_list):
        ...
        sample_model = get_model(data_list[0].name)
    ...
    def draw_plot(self): #,bcenter,mean_flux,mean_err,resid,components):
        ...
        try: ### plot using chips, should work within sherpa
            from pychips import *  
            """ Make the spectral plots using CHIPS
            """
            add_window(10,6,"inches")
            add_frame()
            # split in two
            grid_objects(1,2,0.0,0.0,1)
            adjust_grid_yrelsize(1,2.3)
            
            # plot data spectrum
            add_curve(self.bcenter, self.mean_flux, self.mean_err)
            log_scale()
            set_curve(["symbol.style", "circle", "symbol.fill", True, "symbol.size", 4])
            set_curve(["err.style", "cap","line.style","noline"])
            limits(X_AXIS,Emin,Emax)
            limits(Y_AXIS,Fmin,Fmax)
            
            # plot model spectrum
            # add_line(etrue_center/1e9,p1.calc(para,etrue_center)*1e9 ,["color","red"])
            add_line(xval,yval[-1] ,["color","red"])
            set_plot_ylabel("Flux cm^{-2}s^{-1} TeV^{-1}")
            
            current_plot("plot2")
            # plot residuals 
            add_curve(self.bcenter, self.resid, resid_err)
            set_curve(["symbol.style", "circle", "symbol.fill", True, "symbol.size", 4])
            set_curve(["err.style", "cap","line.style","noline"])
            add_hline(0)
            log_scale(X_AXIS)
            ...
            limits(Y_AXIS,-3.8,3.8) 
            set_plot_ylabel("Residuals (\sigma)")
            set_plot_xlabel("Energy (TeV)")    
                  
        except ImportError:                 # Otherwise, use matplotlib
