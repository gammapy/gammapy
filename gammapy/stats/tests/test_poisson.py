...
def test_docstring_examples():
    ...
    assert_allclose(poisson.background(n_off=4, alpha=0.1), 0.4)
    assert_allclose(poisson.background(n_off=9, alpha=0.2), 1.8)

    assert_allclose(poisson.background_error(n_off=4, alpha=0.1), 0.2)
    assert_allclose(poisson.background_error(n_off=9, alpha=0.2), 0.6)

    assert_allclose(poisson.excess(n_on=10, n_off=20, alpha=0.1), 8.0)
    assert_allclose(poisson.excess(n_on=4, n_off=9, alpha=0.5), -0.5)

    assert_allclose(poisson.excess_error(n_on=10, n_off=20, alpha=0.1), 3.1937439)
    assert_allclose(poisson.excess_error(n_on=4, n_off=9, alpha=0.5), 2.5)
    ...
    actual = poisson.significance(n_observed=1300, mu_background=1100, method='lima')
    assert_allclose(actual, 5.8600870406703329)
    actual = poisson.significance_on_off(n_on=1300, n_off=1100 / 1.e-8, alpha=1e-8, method='lima')
