...
def _x_log_center(xmin, xmax, function=None, table=None):
    ...
    """
    # gmean is not compatible with arrays, so np.sqrt used instead
    x = np.sqrt(xmin * xmax)
    return x
...
def _integrate(xmin, xmax, function, segments=1e3):
    ...
    # Division by number of segments required for normalization
    return np.trapz(y_vals) / segments
