...
class SpectrumGrouping(object):
    ...
    def define_spectral_groups(self, OffsetRange=[0, 2.5], NOffbin=25, EffRange=[0, 100], NEffbin=40,
        ...
        #Ecrire une methode dans SpecobservationList qui le fait pour moi qui retourne la list dpbservation que pour les obs dans la band en utilisant notammnet la fonction get_obs_id. Changer la method het_obs_id pour qu'au lieu de prendre une seule id ca prendra une liste d id
        for nband in range(Nband):
            ...
            if len(tablegroup) == 0:
                ...
                #Il y a quand même peut etre quelque chose de mieux à faire qu une boucle sur les observation dans la abnd pour creer la spectrum_observation_list que je vais donner à apply grouping?
                ...
                ObsBand=SpectrumObservation.apply_grouping(spectrum_observation_band_list, self.ebounds, nband)
                observation_band_list.append(ObsBand)
        self.spectrum_band_list=SpectrumObservationList(observation_band_list)
