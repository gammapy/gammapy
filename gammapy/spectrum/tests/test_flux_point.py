    ...
    try:
        ...
        test_cases.append(
            dict(model=PowerLaw(index=Quantity(2, ''),
                 ...
                 obs=obs(),
                 seg=seg(obs()),
                 dnde = 2.746487655049709e-11 * u.Unit('cm-2 s-1 TeV-1'),
                 dnde_err = 4.755409742841381e-12 * u.Unit('cm-2 s-1 TeV-1'),
                 res = -0.11081311544196776,
                 res_err = 0.15395838267321502,  
                )
        )
        return test_cases
    except IOError:
