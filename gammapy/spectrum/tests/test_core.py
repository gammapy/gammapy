...
from gammapy.modeling.models import PowerLawSpectralModel, PowerLaw2, TableModel
...
def get_test_cases():
    ...
    return [
        dict(model=PowerLawSpectralModel(amplitude="1e2 TeV-1"), e_true=e_true, npred=999),
        dict(
        ...
        dict(
            model=PowerLawSpectralModel(reference="1 GeV", amplitude="1e-11 GeV-1 cm-2 s-1"),
            aeff=EffectiveAreaTable.from_parametrization(e_true),
            livetime="30 h",
            npred=4.34417881,
        ),
