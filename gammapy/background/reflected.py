...
class ReflectedRegionsFinder(object):
    ...
    TODO: Only works for circles at the moment.
    ...
    >>> regions = finder.run(region=on_region, center=pointing)
    >>> print(regions[0])
    ...
    def __init__(self, angle_increment='0.1 rad', min_distance='0 rad',
    ...
    def setup(self, region, center):
        ...
        self._pix_region = region.to_pixel(wcs)
        self._pix_center = PixCoord(*center.to_pixel(wcs))
...
class ReflectedRegionsBackgroundEstimator(object):
    ...
    on_region : `~regions.CircleSkyRegion`
        Target region
    kwargs : dict
        Forwarded to :func:`gammapy.background.ReflectedRegionsFinder`
    """

    def __init__(self, on_region, **kwargs):
        self.on_region = on_region
        self.finder = ReflectedRegionsFinder(**kwargs)
    ...
    def process(self, obs):
        ...
        self.finder.run(region=self.on_region,
    ...
    def plot(self, obs_list, result, fig=None, ax=None, cmap=None, idx=None):
        """Debug plot
