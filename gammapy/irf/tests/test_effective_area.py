...
class TestEffectiveAreaTable2D:
    ...
    def test(aeff):
        ...
        area = aeff.evaluate(energy_true=energy, offset=offset)

        energy_axis_true = MapAxis.from_energy_edges(e_axis, name="energy_true")
        effarea1d = EffectiveAreaTable(axes=[energy_axis_true], data=area.value, unit=area.unit)

        actual = effareafrom2d.evaluate(energy_true="2.34 TeV")
        desired = effarea1d.evaluate(energy_true="2.34 TeV")
        assert_equal(actual, desired)

        # Test ARF export #2
        offset = 1.2 * u.deg
        actual = aeff.to_effective_area_table(offset=offset).quantity
        desired = aeff.evaluate(offset=offset)
        assert_allclose(actual, desired.squeeze(), rtol=1e-9)
...
class TestEffectiveAreaTable:
    ...
    @requires_dependency("matplotlib")
    @requires_data()
    def test_effective_area_table(tmp_path, aeff):
        arf = aeff.to_effective_area_table(offset=0.3 * u.deg)

        assert_quantity_allclose(arf.evaluate(), arf.quantity)
        ...
        assert_quantity_allclose(arf.quantity, arf2.quantity)

        test_aeff = 0.6 * arf.max_area
        node_above = np.where(arf.quantity > test_aeff)[0][0]
        energy = arf.axes["energy_true"]
        ...
        aeff = EffectiveAreaTable(data=data, axes=[energy_axis_true])
...
def test_compute_thresholds_from_parametrization():
    ...
    e_max = aeff.axes["energy_true"].edges[-1]
