...
def test_significance_map_estimator_map_dataset(simple_dataset):
    ...
    assert_allclose(result["norm_sensitivity"].data[0, 10, 10], 38.578027, atol=1e-3)
    assert_allclose(result["sensitivity"].data[0, 10, 10], 3.819225e-10, atol=1e-3)
...
def test_significance_map_estimator_map_dataset_exposure(simple_dataset):
    ...
    assert_allclose(result["sensitivity"].data[0, 10, 10], 4.531563e-09, rtol=1e-3)
