...
from gammapy.data import GTI, HDUIndexTable, DataStore, ObservationTable
...
def test_fit_hawc():
    ...
    event_types = np.arange(5,10) # do only high bins
    ...
    for idx, bin_id in enumerate(event_types): #loop over nhit bins
        ...
        dataset_empty = MapDataset.from_geoms(geom=geom, name="nHit-" + str(bin_id),geom_exposure=geom_exposure,
                            geom_psf=geom_psf,
        ...
        transit_map = Map.read('$GAMMAPY_DATA/hawc/crab_events_pass4/irfs/TransitsMap_Crab.fits.gz')
